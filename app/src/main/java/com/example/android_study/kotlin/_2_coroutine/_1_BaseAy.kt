    ...
    override fun initViewAndData(savedInstanceState: Bundle?) {
        ...
        button4.setOnClickListener {
            lifecycleScope.launch { // this: CoroutineScope
                launch {
                    delay(200L)
                    LogUtil.d("","Task from runBlocking")
                }

                coroutineScope { // 创建一个协程作用域
                    launch {
                        delay(500L)
                        LogUtil.d("","Task from nested launch")
                    }

                    delay(100L)
                    LogUtil.d("","Task from coroutine scope")// 这一行会在内嵌 launch 之前输出
                }
                LogUtil.d("","Coroutine scope is over") // 这一行在内嵌 launch 执行完毕后才输出
            }
        }
    }
}